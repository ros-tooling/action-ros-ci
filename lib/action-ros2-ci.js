...
EOF`], options);
            ...
            yield exec.exec("bash", ["-c", "source install/local_setup.bash && env"], options);
            yield exec.exec("bash", [
                "-c",
                `source install/local_setup.bash && export DYLD_LIBRARY_PATH=/Users/runner/runners/2.160.2/work/system_metrics_collector/system_metrics_collector/ros2_ws/install/rmw_fastrtps_cpp/lib/ && colcon test --event-handlers console_cohesion+ --pytest-args '--cov=.' '--cov-report=xml' --packages-select ${packageName} --return-code-on-test-failure ${extra_options.join(' ')}`,
            ], options);
